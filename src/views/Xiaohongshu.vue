    ...
    <el-card class="xiaohongshu-card" shadow="hover">
      ...
      <div v-if="generatedContent" class="result-section">
        ...
        <div class="content-result">
          <p
            v-for="(paragraph, index) in generatedContent.split('\n')"
            :key="index"
        ...
        <div class="action-buttons">
          <el-button type="success" @click="copyContent" icon="DocumentCopy"
            >复制内容</el-button
          >
          <el-button type="warning" @click="saveContent" icon="FolderOpened"
            >保存内容</el-button
          >
...
const generateContent = () => {
  ...
  setTimeout(() => {
    ...
    const sampleContent = `【${form.topic} - ${
      topics[form.style as keyof typeof topics] || "内容分享"
    }】
...
首先，我们需要了解${form.topic}的基本要点。在进行${
      form.topic
    }时，有几个关键因素需要注意：首先是准备工作，其次是执行过程，最后是后续维护。

准备工作方面，我们需要准备好相关的工具和材料。执行过程中，要特别注意细节处理，这样才能达到最佳效果。

通过我的经验分享，希望能帮助大家更好地了解${
      form.topic
    }。如果你也有相关经验，欢迎在评论区分享交流！

#${form.topic.replace(/\s+/g, "")} #${
      topics[form.style as keyof typeof topics] || "分享"
    } #经验分享`;
